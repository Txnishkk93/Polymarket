...
import { getMarket, placeOrder, derivePrices } from "../lib/api";
...
import { ArrowLeft, ArrowUpDown, Info, AlertCircle, TrendingUp } from "lucide-react";
...
export const MarketDetail: React.FC = () => {
  ...
  const generateChartData = (currentYes: number) => {
    // Generate a beautiful mock history leading up to currentYes
    const startValue = 50;
    const dataPoints = 12;
    const history = [];
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      val = Math.max(1, Math.min(99, val)); // bound check
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <CardHeader className="flex flex-row items-center justify-between py-4">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-yes">
                    {yesPrice}¢
                  </span>
              ...
              <div className="text-xs text-text-secondary bg-surface border border-border px-3 py-1 rounded-md">
                Live
              </div>
            </CardHeader>
                  ...
                  >
                    <defs>
                      <linearGradient id="colorPrice" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#e15241" stopOpacity={0.2} />
                        <stop offset="95%" stopColor="#e15241" stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <XAxis
                      dataKey="date"
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: "#90949d", fontSize: "11px" }}
          ...
          <Card className="bg-surface border-border">
            <CardHeader>
              <CardTitle className="text-base font-bold flex items-center gap-2">
                <ArrowUpDown className="h-4 w-4 text-purple" />
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1 max-h-[200px] overflow-y-auto pr-1">
                  {yesAsks.length === 0 ? (
                    <div className="text-xs text-text-secondary py-3 text-center">
                    ...
                    yesAsks.map((ask, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between text-sm py-1.5 px-2.5 rounded bg-yes/5 border border-yes/10 hover:border-yes/30 transition-all"
                      >
                ...
                <div className="space-y-1 max-h-[200px] overflow-y-auto pr-1">
                  {noAsks.length === 0 ? (
                    ...
                    noAsks.map((ask, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between text-sm py-1.5 px-2.5 rounded bg-no/5 border border-no/10 hover:border-no/30 transition-all"
                      >
              ...
              {isAuthenticated ? (
                <form onSubmit={handleOrderSubmit} className="space-y-4">
                  ...
                  <Tabs value={tradeType} onValueChange={(v: any) => setTradeType(v)}>
                    <TabsList className="bg-neutral-900 border-border grid grid-cols-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`py-3 rounded-lg text-sm font-bold border flex flex-col items-center gap-0.5 transition-all duration-200 ${
                        outcome === "yes"
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[5, 25, 100].map((amount) => (
                        <button
                          ...
                          className="bg-neutral-900 border border-border hover:bg-neutral-800 hover:border-neutral-700 py-1.5 rounded text-xs font-bold text-text-secondary hover:text-white transition-all"
                        >
                          ${amount}
                        </button>
                      ))}
                  ...
                  <div className="bg-neutral-900/40 border border-border/60 rounded-xl p-3.5 space-y-2 text-xs">
                  ...
                  <Button
                    ...
                    className="w-full text-sm py-2.5 font-bold shadow-lg"
                    loading={submitting}
                  >
                ...
                <div className="py-8 text-center space-y-4">
                  <div className="h-10 w-10 rounded-full bg-purple/10 flex items-center justify-center mx-auto text-purple">
                  ...
                  <Link to="/login" className="block">
                    <Button variant="purple" className="w-full text-xs py-2">
                      Log In to Trade
                    </Button>
                  </Link>
                </div>
