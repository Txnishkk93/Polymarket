...
import { getMarkets, getPositions, getHistory, split, merge, onramp, offramp } from "../lib/api";
import type { Market, Position, OrderHistoryItem } from "../lib/api";
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">
          Manage your funds, view open positions, and run share operations
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card className="bg-surface border-border overflow-hidden relative">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  variant="purple"
                  className="w-full text-xs flex items-center gap-1 py-2"
                  onClick={() => {
                    setAmountInput("");
                    setModalError(null);
                ...
                <Button
                  variant="outline"
                  className="w-full text-xs flex items-center gap-1 py-2 hover:bg-neutral-800"
                  onClick={() => {
                    setAmountInput("");
                    setModalError(null);
          ...
          <Card className="bg-surface border-border">
            ...
            <CardContent className="space-y-4">
              <div className="text-xs text-text-secondary leading-relaxed bg-neutral-900/40 p-3 rounded-lg border border-border/50">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="flex h-10 w-full rounded-lg bg-surface border border-border px-3 py-2 text-sm text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200"
                >
                  {markets.map((market) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  variant="outline"
                  className="w-full text-xs font-bold py-2 hover:bg-neutral-800"
                  onClick={handleSplit}
        ...
        <div className="lg:col-span-8">
          <Tabs defaultValue="positions">
            <TabsList className="grid grid-cols-2 max-w-[400px] bg-neutral-900 border-border">
              <TabsTrigger value="positions" className="flex items-center gap-1 text-xs">
                ...
                <CardContent className="p-0">
                  {positions.filter((p) => p.qty > 0).length === 0 ? (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="w-[60%]">Market Title</TableHead>
                          <TableHead>Outcome</TableHead>
                          <TableHead className="text-right">Shares (Qty)</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {positions
                ...
                <CardContent className="p-0">
                  {history.length === 0 ? (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="w-[50%]">Market</TableHead>
      ...
      <Dialog isOpen={isOnrampOpen} onClose={() => setIsOnrampOpen(false)}>
        <form onSubmit={handleOnramp}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" variant="purple" loading={modalLoading}>
              Deposit Funds
            </Button>
          </DialogFooter>
        </form>
